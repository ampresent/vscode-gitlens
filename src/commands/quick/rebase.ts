...
export class RebaseQuickCommand extends GitCommandBase {
    ...
    execute(state: State) {
        runGitCommandInTerminal('rebase', state.source.ref, state.repo.path);
    }
            ...
            try {
                ...
                const step = this.createConfirmStep(
                    ...
                    [
                        {
                            label: this.title,
                            description: `$(git-branch) ${state.destination.name} with${Strings.pad(
                                '$(git-branch)',
                                2,
                                1
                            )}${state.source.name}`,
                            detail: `Will update${Strings.pad('$(git-branch)', 2, 1)}${
                                state.destination.name
                            } by applying x commits on top of${Strings.pad('$(git-branch)', 2, 1)}${state.source.name}`
                        }
                    ]
                );
                const selection = yield step;
