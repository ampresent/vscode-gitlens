            ...
            try {
                ...
                if (state.commits === undefined || state.counter < 4) {
                    ...
                    const step = this.createStep<CommitQuickPickItem>({
                        ...
                        multiselect: true,
                        placeholder: `Choose commits to cherry-pick onto ${state.destination.name}`,
                        items: [
                ...
                const step = this.createConfirmStep(
                    ...
                    [
                        {
                            label: this.title,
                            description: `${Strings.pluralize('commit', state.commits.length)} onto${Strings.pad(
                                '$(git-branch)',
                                2,
                                1
                            )}${state.destination.name}`,
                            detail: `Will apply ${Strings.pluralize('commit', state.commits.length)} onto${Strings.pad(
                                '$(git-branch)',
                                2,
                                1
                            )}${state.destination.name}`
